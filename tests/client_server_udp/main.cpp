...
// The test creates a UDP server listening on all interfaces mPort 5555
...
Kissnet::UDPSocket server_socket;
...
void client_1(const std::string& target_ip) {
    ...
    Kissnet::UDPSocket client1_socket(Kissnet::Endpoint(target_ip, 5555));
    ...
    if (received_bytes == 16 && status == Kissnet::SocketStatus::VALID) {
...
void client_2(const std::string& target_ip) {
    ...
    Kissnet::UDPSocket client2_socket(Kissnet::Endpoint(target_ip, 5555));
...
void server(const std::string& server_ip) {
    ...
    Kissnet::UDPSocket new_server_socket(Kissnet::Endpoint(server_ip, 5555));
    server_socket = std::move(new_server_socket);
    server_socket.bind();
    server_running = true;
    while (true) {
        Kissnet::AddrCollection addr;
        auto[received_bytes, status] = server_socket.recv(recv_buff, 0, &addr);
        if (!received_bytes || status != Kissnet::SocketStatus::VALID) {
            break;
        }
